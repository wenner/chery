		...
		return {
			...
			link : function(scope, element, attrs, ngModel) {
				...
				function renderScrollableTabs(){
					...
					angular.forEach(icons, function(value, key){
                        ...
                        tabsWidth += value.getBoundingClientRect().width;
					});

                    var indicator = angular.element(document.querySelector(".tsb-icons .scroll .tab-indicator-wrapper"));
                    indicator.css("width",tabsWidth + icons.length * 4);
				...
				scope.setIndicatorPosition = function(index, position) {
					var slideTabs = angular.element(document.querySelector(".tsb-icons .scroll")).find("a");
					var indicator = angular.element(document.querySelector(".tsb-icons .scroll .tab-indicator"));
					var targetSlide = angular.element(slideTabs[index]);
					var targetSlideMarginLeft = targetSlide.css("margin-left");

					var currentSlideIndex = index;
					if(position < 1 && index - 1 >= 0) {
						currentSlideIndex = index - 1;
					}
					else if (position > 1 && index + 1 <= slideTabs.length - 1) {
						currentSlideIndex = index + 1;
					}
					var currentSlide = angular.element(slideTabs[currentSlideIndex]);

					var targetLeftOffset = targetSlide.prop("offsetLeft");
					var currentLeftOffset = currentSlide.prop("offsetLeft");
					var offsetLeftDiff = Math.abs(targetLeftOffset - currentLeftOffset);

					var targetWidth = targetSlide[0].getBoundingClientRect().width;
					var currentWidth = currentSlide[0].getBoundingClientRect().width;
					var widthDiff = targetWidth - currentWidth;

					

					if(position > 1) {
						indicator.css("left",targetLeftOffset + (offsetLeftDiff * (position-1)) + "px");
						indicator.css("width",targetWidth + ((widthDiff * (1 - position))) + "px");
					}
					else if(position < 1) {
						indicator.css("left",targetLeftOffset - (offsetLeftDiff * (1 - position)) + "px");
						indicator.css("width",targetWidth + ((widthDiff * (position-1))) + "px");
					}
                    else if(position == 1) {
			...
			controller : function($scope, $attrs, $element) {
				...
				$scope.slideHasChanged = function(index){
                    ...
                    $scope.setIndicatorPosition(index,1);
				};

				$scope.slidePositionChanged = function(index, position) {
                    $scope.events.trigger("slideChange", {"index" : index});
					$scope.setIndicatorPosition(index, position);
				}
